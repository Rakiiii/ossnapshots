...
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    ...
    // LAB 6: Your code here
    uintptr_t lookup_addr = start;

    while(lookup_addr < end) {
        ...
        for(; class >= 0; class--) {
            if(page_lookup(NULL, lookup_addr, class, type, 1)) break;
        }

        lookup_addr += CLASS_SIZE(class);
    }
}
...
dump_virtual_tree(struct Page *node, int class) {
    ...
    if ((node->state & NODE_TYPE_MASK) == MAPPING_NODE) {
        cprintf("Node dumped with physical %p mapped to virtual %d\n", (void *)PTE_ADDR(node->phy->addr), class);
    }
...
dump_memory_lists(void) {
    ...
    for(class = 0; class < MAX_CLASS; class++) {
        ...
        struct List *node = free_classes[class].next;

        while (node != &free_classes[class]) {
            cprintf("address %p for class %d\n",(void *)page2pa((struct Page *)node), class );
            node = node->next;
        }
    }
}
...
dump_page_table(pte_t *pml4) {
    ...
    for(int index_pdp = NUSERPML4; index_pdp < PML4_ENTRY_COUNT; index_pdp++) {

        pdpe_t *pdp = (pdpe_t *)KADDR(PTE_ADDR(pml4[index_pdp]));

        for(int index_pd = 0; index_pd < PDP_ENTRY_COUNT; index_pd++) {

            if (pdp[index_pd] & PTE_PS) {
                dump_entry(pdp[index_pd], 1 * GB, 0);
                continue;
            }

            if (!(pdp[index_pd] & PTE_P)) {
				continue;
			}

            pde_t *pd = (pde_t *) KADDR(PTE_ADDR(pdp[index_pd]));

            for(int index_pt = 0; index_pt < PD_ENTRY_COUNT; index_pt++) {

                if (pd[index_pt] & PTE_PS) {
                    dump_entry(pd[index_pt], 2 * MB, 0);
                    continue;
                }

                if (!(pd[index_pt] & PTE_P)) {
					continue;
				}

                pte_t *pt = (pte_t *) KADDR(PTE_ADDR(pd[index_pt]));
                for (int index_pt_internal = 0; index_pt_internal < PT_ENTRY_COUNT; index_pt_internal++) {
                    if (!(pt[index_pt_internal] & PTE_P)) {
						continue;
					}
                    dump_entry(pt[index_pt_internal], 4 * KB, 0);
                }
...
switch_address_space(struct AddressSpace *space) {
    ...
    struct AddressSpace *switched_space = current_space;
    current_space = space;
    lcr3(current_space->cr3);
    return switched_space;
}
...
init_memory(void) {
    ...
    if(map_physical_region(&kspace,KERN_BASE_ADDR,0,max_memory_map_addr, PROT_R | PROT_W | ALLOC_WEAK) != 0) {
        panic("Physical region cannot be mapped from %d sized %zd\n",0,max_memory_map_addr);
    }
    ...
    // LAB 7: Your code here
    // Map [PADDR(__text_start);PADDR(__text_end)] to [__text_start, __text_end] as RW- !!!MISTAKE!!! SHOULD BE RX
    if(map_physical_region(&kspace,(uintptr_t)__text_start, PADDR(__text_start), (size_t)(__text_end - __text_start), PROT_R | PROT_W | PROT_X) != 0) {
        panic("Physical region cannot be mapped from %lu sized %zd\n",(uintptr_t)__text_start,(size_t)(__text_end - __text_start));
    }
    ...
    if(map_physical_region(&kspace,KERN_STACK_TOP - KERN_STACK_SIZE, PADDR(bootstack),  KERN_STACK_SIZE, PROT_R | PROT_W) != 0) {
        panic("Physical region cannot be mapped from %p sized %lld\n", (void *)PADDR(bootstack),KERN_STACK_SIZE);
    }

    if(map_physical_region(&kspace,KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE, PADDR(pfstack),  KERN_PF_STACK_SIZE, PROT_R | PROT_W) != 0) {
        panic("Physical region cannot be mapped from %p sized %lld\n",(void *)PADDR(pfstack),KERN_PF_STACK_SIZE);
    }
    ...
    for (; mstart < mend; mstart = (void *)((uint8_t *)mstart + uefi_lp->MemoryMapDescriptorSize)) {
        if (mstart->Attribute & EFI_MEMORY_RUNTIME) {
            ...
            if(map_physical_region(&kspace,mstart->VirtualStart, mstart->PhysicalStart, mstart->NumberOfPages * PAGE_SIZE, PROT_R | PROT_W) != 0) {
                panic("Physical region cannot be mapped from %lu sized %llu\n",mstart->PhysicalStart, mstart->NumberOfPages * PAGE_SIZE);
            }
        }
    }
    ...
    * This must be done as early as possible */
    ...
    if (map_physical_region(&kspace, FRAMEBUFFER, uefi_lp->FrameBufferBase, (uintptr_t)uefi_lp->FrameBufferSize, PROT_R | PROT_W | PROT_WC) != 0) {
        panic("Physical region cannot be mapped from %p sized %zd\n", (void *)uefi_lp->FrameBufferBase, (uintptr_t)uefi_lp->FrameBufferSize);
    }
    if (map_physical_region(&kspace, X86ADDR(KERN_BASE_ADDR), 0, (uintptr_t)MIN(MAX_LOW_ADDR_KERN_SIZE, max_memory_map_addr), PROT_R | PROT_W | ALLOC_WEAK) != 0) {
        panic("Physical region cannot be mapped from %p sized %zd\n", (void *)0, (uintptr_t)MIN(MAX_LOW_ADDR_KERN_SIZE, max_memory_map_addr));
    }
    if (map_physical_region(&kspace, X86ADDR((uintptr_t)__text_start), PADDR(__text_start), (uintptr_t)ROUNDUP(X86ADDR((uintptr_t)__text_end), CLASS_SIZE(0)) - X86ADDR((uintptr_t)__text_start), PROT_R | PROT_W  | PROT_X) != 0) {
        panic("Physical region cannot be mapped from %p sized %zd\n", (void *)PADDR(__text_start), (uintptr_t)ROUNDUP(X86ADDR((uintptr_t)__text_end), CLASS_SIZE(0)) - X86ADDR((uintptr_t)__text_start));
    }
    if (map_physical_region(&kspace, X86ADDR(KERN_STACK_TOP - KERN_STACK_SIZE), PADDR(bootstack), (uintptr_t)KERN_STACK_SIZE, PROT_R | PROT_W) != 0) {
        panic("Physical region cannot be mapped from %p sized %zd\n", (void *)PADDR(bootstack), (uintptr_t)KERN_STACK_SIZE);
    }
    if (map_physical_region(&kspace, X86ADDR(KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE), PADDR(pfstack), (uintptr_t)KERN_PF_STACK_SIZE, PROT_R | PROT_W) != 0) {
        panic("Physical region cannot be mapped from %p sized %zd\n", (void *)PADDR(pfstack), (uintptr_t)KERN_PF_STACK_SIZE);
    }
